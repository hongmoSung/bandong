...
<div class="container">
	...
	<div class="center-block" style="width: 500px;">
			...
			<div class="form-group">
				<c:choose>
					<c:when test="${not empty authEmail}">
							...
							<div class="col-sm-9">
								<input type="text" id="a"><button type="button" onclick="auth()" class="btn btn-primary btn-xl" style="margin-left: 5px;">인증하기</button><br>
							</div>
					...
					<c:otherwise>
							...
							<div class="col-sm-9">
								<input type="text" name="authMail" id="authMail"><button class="btn btn-primary btn-xl" style="margin-left: 5px;">메일인증</button>
							</div>
					</c:otherwise>
				</c:choose>
			</div>
		...
		<form class="text-center" action="joinForm" method="post">
			...
			<button hidden="true" id="joinButton" class="btn btn-primary btn-xl">가입하러가기</button>
		</form>
		</div>
	</div>
	</div>
	...
	<script>
		...
		if(result.value != "") {
			if(result.value == "성공") {
				swal("인증번호가 전송되었습니다.", "메일을 확인해주세요.", "success");
			}
			if(result.value == "실패") {
		...
		function auth() {
			...
			else if (inputNum.value == authNum.value && authNum.value != "") {
